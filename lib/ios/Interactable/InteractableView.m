...
{
    CGPoint translation = [pan translationInView:self];
    // Hirebotics - Only capture the gesture swipes to the left so that this package does not interfere
    //    with a left sided drawer layout
    if (self.horizontalOnly) return fabs(translation.x) > fabs(translation.y) && translation.x < 0;
